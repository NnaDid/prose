...
class GetFeMalePelvicCancerByDate{
    use Common;
    ...
    public function getSideEffectsByDate(){
        ...
        $dateLogged = $obj['date'];
        $query      = $con->query("SELECT * FROM `female_pelvic_effect` WHERE `userId` ='$userId' AND `dateLogged`='$dateLogged'");
        if($query){
            if($query ->num_rows>0){
...
new GetFeMalePelvicCancerByDate();
